...
exports.deleteProduct = (req, res)=>{
    if(req.productById){
        ...
        product.remove((err, product)=>{
            if(err) return res.status(400).json({error: 'Product Not Deleted...!'})
...
exports.productByCatId = (req, res, next, id)=>{
    Product.find({categoryId: ObjectId(id)}, (err, products)=>{
        ...
        if(err) return res.status(404).json({error: 'Category Not Exist with this Id'})
...
exports.updateProduct = (req, res) =>{
    ...
    form.parse(req, (err, fields, files)=>{
        if(err) return res.status(400).json({error: 'failed to parse update request'})
...
exports.createProduct = (req, res)=>{
    ...
    form.parse(req, (err, fields, files)=>{
        ...
        if(files.img){
            ...
            product.contentType = files.img.type
        }
